...
@ActiveProfiles("test")
@Transactional
@DisplayName("Tests de Integración - RegistroController")
class RegistroControllerIntegrationTest {
    ...
    @Test
    @DisplayName("POST / - Debe crear registro y retornar 201 Created")
    void crearRegistro_Success() throws Exception {
        // 1. Creamos la solicitud (Nace como PENDIENTE)
        ...
        Long solicitudId = objectMapper.readValue(resultSolicitud.getResponse().getContentAsString(), SolicitudArriendoDTO.class).getId();

        // 2. ACEPTAMOS la solicitud (El paso que faltaba)
        mockMvc.perform(patch(SOLICITUD_URL + "/{id}/estado", solicitudId)
                        .param("estado", "ACEPTADA"))
                .andExpect(status().isOk());

        // 3. Ahora sí, intentamos crear el registro
